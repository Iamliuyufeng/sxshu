...
const CommonTablePage = ({
  ...
  // 1. 首次加载/查询按钮触发
  const handleSearch = () => {
    fetchTableData(requestUrl, query, { pageSizeKey, currentKey }) // 传入查询条件和分页参数名
  }
  ...
  const handlePageChange = (newCurrent, newPageSize) => {
    if (newCurrent !== current) {
      ...
      fetchTableData(requestUrl, query, { pageSizeKey, currentKey, page: newCurrent }) // 直接传 page 值
    }
    if (newPageSize !== pageSize) {
      changeTablePageSize(newPageSize) // 先更新页大小
      fetchTableData(requestUrl, query, { pageSizeKey, currentKey, page: 1 }) // 重置到第一页
    }
  }
  ...
  const handleRefresh = () => {
    refreshTable()
    fetchTableData(requestUrl, query, { pageSizeKey, currentKey }) // 重新加载
  }
